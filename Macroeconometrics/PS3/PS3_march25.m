...
grsp=zeros(length(sp),1); %predeine growth rate of stock prices
for i=2:length(sp)
    grsp(i,1)=log(sp(i)/sp(i-1,1));
end

%create the lags of stock prices and TFP
...
S=chol(varcov,'lower');
%eta=S\ehat';
eta=inv(S)*ehat'; %Gabor: according to the slides
...
for i=1:1000
    ...
    D=C*S2b(:,:,i);
    x=fsolve(@(x) D(2,1)*sin(x)+D(2,2)*cos(x), 0);
...
for i=1:1000
    ...
    Y3=[TFPlags(5:length(yhat3),1) SPlags(5:length(yhat3),1) GGDPlags(5:length(yhat3),1) GRCOlags(5:length(yhat3),1)];
    cons=ones(length(yhat3)-4,1);
    X3=[cons TFPlags(5:length(yhat3),2) SPlags(5:length(yhat3),2) GGDPlags(5:length(yhat3),2) GRCOlags(5:length(yhat3),2) ... first lag
        TFPlags(5:length(yhat3),3) SPlags(5:length(yhat3),3) GGDPlags(5:length(yhat3),3) GRCOlags(5:length(yhat3),3) ... second lag
        TFPlags(5:length(yhat3),4) SPlags(5:length(yhat3),4) GGDPlags(5:length(yhat3),4) GRCOlags(5:length(yhat3),4) ... third lag
        TFPlags(5:length(yhat3),5) SPlags(5:length(yhat3),5) GGDPlags(5:length(yhat3),5) GRCOlags(5:length(yhat3),5) ];  %fourth lag
...
%1. perform an ADF test to each variable to determine if it is stationary
%   or not. If it is not stationary, take first diference and take logs
...
%
%    Workbook:
%    /Users/camilojosepechagarzon/Documents/MATLAB/Macroeconometric/PS3/Data.xlsx
%    Worksheet: Foglio1
%
...
%% Import the data
[~, ~, raw] = xlsread('/Users/camilojosepechagarzon/Documents/MATLAB/Macroeconometric/PS3/Data.xlsx','Foglio1');
raw = raw(3:214,:);
...
Y=[TFPlags(5:obs1,1) SPlags(5:obs1,1) GGDPlags(5:obs1,1) GRCOlags(5:obs1,1) PC1(5:obs1,1) PC2(5:obs1,1) PC3(5:obs1,1) PC4(5:obs1,1)];
cons=ones(length(SPlags)-4,1);
X=[cons TFPlags(5:obs1,2) SPlags(5:obs1,2) GGDPlags(5:obs1,2) GRCOlags(5:obs1,2) lPC1(5:obs1,1) lPC2(5:obs1,1) lPC3(5:obs1,1) lPC4(5:obs1,1) ... first lag
        TFPlags(5:obs1,3) SPlags(5:obs1,3) GGDPlags(5:obs1,3) GRCOlags(5:obs1,3) slPC1(5:obs1,1) slPC2(5:obs1,1) slPC3(5:obs1,1) slPC4(5:obs1,1) ... second lag
         ];  %fourth lag
...
for i=1:1000
    ...
    Y3=[TFPlags(5:length(yhat3),1) SPlags(5:length(yhat3),1) GGDPlags(5:length(yhat3),1) GRCOlags(5:length(yhat3),1) PC1lags(5:length(yhat3),1) PC2lags(5:length(yhat3),1) PC3lags(5:length(yhat3),1) PC4lags(5:length(yhat3),1)];
    cons=ones(length(yhat3)-4,1);
    X3=[cons TFPlags(5:length(yhat3),2) SPlags(5:length(yhat3),2) GGDPlags(5:length(yhat3),2) GRCOlags(5:length(yhat3),2) PC1lags(5:length(yhat3),2) PC2lags(5:length(yhat3),2) PC3lags(5:length(yhat3),2) PC4lags(5:length(yhat3),2) ... first lag
             TFPlags(5:length(yhat3),3) SPlags(5:length(yhat3),3) GGDPlags(5:length(yhat3),3) GRCOlags(5:length(yhat3),3) PC1lags(5:length(yhat3),3) PC2lags(5:length(yhat3),3) PC3lags(5:length(yhat3),3) PC4lags(5:length(yhat3),3)]; %second lag
    phi2=inv(X3'*X3)*X3'*Y3;
    Yhat2=X3*phi2;
    %residuals
    ehat2=Yhat2-Y3;
